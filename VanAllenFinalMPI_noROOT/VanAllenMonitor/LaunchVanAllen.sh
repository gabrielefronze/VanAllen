...
Multi=$(cat VanAllenConfig.config | grep -i '^Tasksize' | grep -Eo '[0-9]{1,}')
...
#mpiexec -np 6 VanAllen 0 400 1 1 1234 1 0 &
echo "mpiexec $MPIoptions VanAllen $NelLow $NelHi $Multi 1 $Seed $Master $Finalizer &"
mpiexec $MPIoptions VanAllen $NelLow $NelHi $Multi 1 $Seed $Master $Finalizer &
